...
export const CurrencyCardCash = (props: CurrencyCardProps) => {
  ...
  return (
    <Card
      className={cx(
        "h-auto relative shadow-none border-0 bg-new-medium-grey rounded-[10px] text-white min-w-0 cursor-pointer",
    ...
    >
      {variant === 0 && currency?.is_popular && (
        <span
          className={cn(
            "absolute right-0 top-0 text-[10px] rounded-tr-[10px] rounded-bl-[10px] bg-mainColor text-black text-center py-[2px] px-2 font-medium",
      ...
      {variant === 1 && currency?.is_popular && (
        ...
        >
          <PinIcon
            className={`${
              active ? "[&>path]:fill-[#131312]" : "[&>path]:fill-mainColor"
            }`}
          />
        </span>
      )}
      {variant === 2 && currency?.is_popular && (
        <span
          className={cn(
            "absolute right-5 top-[50%] -translate-y-[50%] font-medium"
          )}
      ...
      <CardContent className="grid h-full grid-cols-[auto_1fr] px-5 py-2 justify-start gap-5 gap-y-0 items-center leading-none">
        ...
        <div className="flex flex-col min-w-0 items-start gap-0">
          <p
            className={`font_unbounded text-sm line-clamp-1 text-start font-semibold uppercase ${
              variant === 0 && "max-w-[80%]"
            }`}
          >
            {currencyName}
